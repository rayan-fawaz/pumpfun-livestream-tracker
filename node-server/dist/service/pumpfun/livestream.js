...
export const cleanUpSeenAddresses = async () => {
    try {
        ...
        console.log(`[${new Date().toISOString()}] Cleanup complete. Remaining active live streams: ${seenAddresses.size}`);
    }
    catch (error) {
...
const checkPumpFunLiveStreams = async () => {
    try {
        ...
        console.error(`[${new Date().toISOString()}] error during API request:`, error);
    }
};
const initializeSeenAddresses = async () => {
    try {
        ...
        console.error(`[${new Date().toISOString()}] Error initializing seen addresses:`, error);
    }
};
